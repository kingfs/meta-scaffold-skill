...
description: 用于为 AI 快速搭建或重构生产级、AI-native 的 Go 后端元工程。适用于需要基于 protobuf 作为接口唯一事实来源、同时产出 gRPC 与 REST 接口、提供 agent 可稳定调用的运维 CLI 命令面、使用 buf/ent/wire/cobra/viper/go-task/GitLab CI/Docker，并要求仓库结构 AI-friendly、强调 DDD 战略分析但避免僵化套用、明确生成代码不可手改、补齐单测与端到端测试的场景。
---
...
- 需要服务二进制同时暴露 AI-native 运维命令面，例如 `serve`、`migrate`、`health`、`config inspect`、`schema`、`version`
...
- `ops_command_surface`：服务二进制需要暴露的运维命令、输出格式、dry-run 和确认边界
...
7. 服务入口采用 `cobra` + `viper`。
   ...
   - 默认至少提供 `serve`、`migrate up`、`migrate down`、`version`，按需提供 `health`、`config inspect`、`schema`。
   ...
   - 根命令应预留 AI-native 全局 flags：`--format json|ndjson|table|pretty|yaml`、`--quiet`、`--no-input`、`--no-color`、`--trace-id`。
   - stdout 只放主结果；stderr 放进度、warning 与诊断。JSON 成功与失败输出都应有稳定 envelope。
   - 有副作用的运维命令必须支持 `--dry-run` 或 preview；高风险命令使用 `--confirm <resource-id>`，不要让 `--yes` 绕过权限或安全策略。
   - `schema` 或机器可读 help 应能让 agent 查询命令参数、输出结构、权限需求与副作用范围。
8. 项目管理采用 `go-task`。
...
10. 补齐 AI 与人类文档。
   ...
   - `ARCHITECTURE.md`：给工程师和 AI 看的架构说明、分层关系、关键链路、运维命令契约。
...
- 不要把服务运维命令设计成只给人看的 table/prose 输出；agent 默认路径必须可结构化解析。
- 不要让日志、spinner、warning 混入 JSON stdout。
- 不要把交互式 prompt 放进自动化路径；CI/agent 路径必须支持 `--no-input` 并在缺参时结构化失败。
- 不要把 secrets、token、cookie、数据库密码、生产连接串输出到 stdout、日志、测试快照或文档。
...
- 服务 CLI 的机器契约明确包含 `--format json`、结构化错误、stdout/stderr 分离、可解释退出码与 dry-run-first 规则
- 运维命令文档明确说明权限、输出 schema、副作用、常见错误和恢复命令
...
7. 检查服务 CLI 是否支持 `--format json`、`--no-input`、`--trace-id`，成功与失败路径是否都有稳定 envelope。
8. 检查 migrate、config、schema 等有副作用或敏感输出命令是否支持 dry-run、脱敏和确认边界。
9. 检查 CI stages 是否与要求一致，且 package 能复用 build 产物。
10. 检查 Docker 运行路径是否满足 `/app/bin` 与 `/app/config` 约定，并能携带 `ent/migrations/**` 进入镜像。
11. 检查测试策略是否覆盖单测与 e2e，并说明 SQLite 与 Postgres 差异。
12. 检查 `Taskfile`、CI、脚本、README 中没有把 `GOCACHE`、`GOMODCACHE`、`GOPATH` 指向仓库内 `.cache/` 或其他项目级缓存目录。
